...
class TextDecoder(nn.Module):
    ...
    @staticmethod
    def _block_ngram_repeats(logits: torch.Tensor, seq: List[int], no_repeat_ngram: int) -> torch.Tensor:
        """Set log-prob of any token that would complete a repeated n-gram to -inf."""
        if no_repeat_ngram < 1 or len(seq) < no_repeat_ngram:
            return logits
        # Collect all (n-1)-grams already seen and which token followed each
        banned: set = set()
        context = tuple(seq[-(no_repeat_ngram - 1):])
        for i in range(len(seq) - no_repeat_ngram + 1):
            if tuple(seq[i: i + no_repeat_ngram - 1]) == context:
    ...
    def generate_greedy(
        ...
        """
        Greedy autoregressive decoding with n-gram repetition blocking.

        Args:
            memory: (B, T, d_model)
            no_repeat_ngram: block any n-gram seen before (0 = disabled).
        ...
        for _ in range(max_len - 1):
            ...
            for b in range(B):
                if not done[b]:
                    step_logits[b] = self._block_ngram_repeats(
                        step_logits[b], generated[b].tolist(), no_repeat_ngram
                    )
        ...
        for b in range(B):
            ...
            for step in range(max_len - 1):
                ...
                for score, seq in beams:
                    ...
                    next_logits = self._block_ngram_repeats(next_logits, seq, no_repeat_ngram)
    ...
    def generate(
        ...
        return self.generate_greedy(
            memory, memory_key_padding_mask, max_len,
            no_repeat_ngram=no_repeat_ngram,
        )
